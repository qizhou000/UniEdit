...
def get_reprs_at_word_tokens(
    ...
    subtoken: str,
    track: str = "in",
    context_template_placeholder: str = ...,
) -> torch.Tensor:
    """
    ...
    idxs = get_words_idxs_in_templates(tok, context_templates, words, subtoken, context_template_placeholder)
    return get_reprs_at_idxs(
        model,
        tok,
        [context_templates[i].replace(context_template_placeholder, words[i]) for i in range(len(words))],
        idxs,
        layer,
        module_template,
        track,
...
def get_words_idxs_in_templates(
    tok: AutoTokenizer, context_templates: str, words: str, subtoken: str, context_template_placeholder
) -> int:
    """
    Given list of template strings, each with *one* format specifier
    (e.g. "`context_template_placeholder` plays basketball"), and words to be substituted into the
    template, computes the post-tokenization index of their last tokens.
    """

    assert all(
        tmp.count(context_template_placeholder) == 1 for tmp in context_templates
    ), "We currently do not support multiple fill-ins for context"


    prefixes_len, words_len, suffixes_len, inputs_len = [], [], [], []
    for i, context in enumerate(context_templates):
        prefix, suffix = context.split(context_template_placeholder)
