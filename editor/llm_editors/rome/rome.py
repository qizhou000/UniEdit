from ... import BaseConfig, nethook
...
class ROME(LLMBaseEditor):

    def __init__(self, llm: BaseLLMForEdit, config:ROMEConfig,
        stats_dir: str = 'data/rome-memit-stats', comput_stat_data_path = None,
        ...
        self.context_templates = get_context_templates(llm.model, llm.tokenizer, self.cfg.context_template_length_params)
        self.inv_mom2_cache = {}
        w_name = f"{self.cfg.rewrite_module_tmp.format(self.cfg.edit_layer)}.weight"
        self.original_w = nethook.get_parameter(self.llm.model, w_name).clone()
        self.verbose = verbose
        if 'llama' in self.cfg.edit_model_name:
            self.model_hidden_size = llm.model.config.hidden_size
        else:
            self.model_hidden_size = llm.model.config.n_embd
        self.subject_placeholder = 'a6-g8+750+d1*-65+8'
    ...
    def edit_one_piece(self, request: Dict) -> None:
        ...
        """
        p = request['prompt'] + ' ' + request['target_new']
        toks = self.llm.tokenizer(p)
        if len(toks['input_ids']) > 150: # OOM
            return 
    ...
    def save_current_edit_status(self):
        ...
        w = nethook.get_parameter(self.llm.model, w_name)
        with torch.no_grad():
            self.saved_edit_status = w.clone()
    ...
    def execute_rome(self, request: Dict) -> None:
        ...
        if self.verbose:
            print(
                ...
                f"[{request['prompt'].replace(self.subject_placeholder, request['subject'])}] -> [{request['target_new']}]"
            )
        deltas = {} 
    ...
    def compute_u(self, request: Dict) -> torch.Tensor:
        ...
        if "subject_" in self.cfg.fact_token and self.cfg.fact_token.index("subject_") == 0:
            ...
            cur_repr = repr_tools.get_reprs_at_word_tokens(
                ...
                context_template_placeholder = self.subject_placeholder,
                **word_repr_args,
            ).mean(0)
        elif self.cfg.fact_token == "last":
            cur_repr = repr_tools.get_reprs_at_idxs(
                contexts=[
                    templ.format(request["prompt"].replace(self.subject_placeholder, request["subject"]))
    ...
    def compute_v(self, request: Dict, left_vector: torch.Tensor) -> torch.Tensor:
        ...
        target_ids = self.llm.tokenizer(request["target_new"], 
            return_tensors="pt").to(self.device)["input_ids"][0]

        # Compile list of rewriting and KL x/y pairs
        rewriting_prompts, kl_prompts = [
            context.format(request["prompt"]) + self.llm.tokenizer.decode(target_ids[:-1])
            for context in self.context_templates
        ], [self.subject_placeholder + " is a"]
        all_prompts = rewriting_prompts + kl_prompts

        input_tok = self.llm.tokenizer(
        ...
        lookup_idxs = [
            self.find_fact_lookup_idx(
                prompt, request["subject"], self.cfg.fact_token
        ...
        nethook.set_requires_grad(False, self.llm.model)
        ...
        for it in range(self.cfg.v_num_grad_steps):
            ...
            with nethook.TraceDict(
                ...
                logits = self.llm.model(**input_tok).logits
    ...
    def get_module_input_output_at_word(self, context_template: str, 
        word: str, module_template: str, fact_token_strategy: str, 
        context_template_placeholder) -> Tuple[torch.Tensor]:
        """
        ...
        if "subject_" in fact_token_strategy and fact_token_strategy.index("subject_") == 0:
            ...
            l_input, l_output = repr_tools.get_reprs_at_word_tokens(
                ...
                words=[word],
                context_template_placeholder = context_template_placeholder,
                **word_repr_args,
            )
        elif fact_token_strategy == "last":
            l_input, l_output = repr_tools.get_reprs_at_idxs(
                track="both",
                contexts=[context_template.replace(context_template_placeholder, word)],
                idxs=[[-1]],
                **word_repr_args,
            )
        else:
