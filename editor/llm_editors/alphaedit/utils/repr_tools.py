...
def get_reprs_at_word_tokens(
    ...
    idxs = get_words_idxs_in_templates(tok, context_templates, words, subtoken, subject_placeholder)
    return get_reprs_at_idxs(
        model,
        tok,
        [context_templates[i].replace(subject_placeholder, words[i]) for i in range(len(words))],
        idxs,
        layer,
        module_template,
        track,
...
def get_words_idxs_in_templates(
    ...
    assert all(
        tmp.count(subject_placeholder) == 1 for tmp in context_templates
    ), "We currently do not support multiple fill-ins for context"


    prefixes_len, words_len, suffixes_len, inputs_len = [], [], [], []
    for i, context in enumerate(context_templates):
        prefix, suffix = context.split(subject_placeholder)
